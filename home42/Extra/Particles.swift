...
final class ParticlesEmitter: CAEmitterLayer {
    ...
    @frozen enum Style {
        ...
        case stars
        case alertStars
        case coalitionHorizontal(SVGKImage, UIColor, Float)
    ...
    func configure(_ style: Style) {
        ...
        case .coalitionHorizontal(let svgImage, let color, let birthRateMultiplier):
            let cell = CAEmitterCell.init()
            let imageView = SVGKFastImageView(svgkImage: svgImage)!
            let image: CGImage!
            
            imageView.frame = .init(origin: .zero, size: .init(width: 25.0, height: 25.0))
            image = imageView.renderImage().cgImage
            self.emitterMode = .outline
