...
//  home42
//
//  Created by Antoine Feuerstein on 06/07/2021.
//
...
final class GraphSharedViewController: HomeViewController {
    ...
    init(user: IntraUser, cursus: IntraUserCursus, coalition: IntraCoalition?) {
        ...
        self.header = ControllerHeaderWithActionsView(asset: .controllerGraph, title: ~"TITLE_GRAPH", primary: primary)
...
final fileprivate class HolyGraphView: BasicUIView, SelectorViewDelegate, SearchFieldViewDelegate, UserSearchFieldViewDelegate, UIScrollViewDelegate, Keyboard {
    ...
    init(user: IntraUser, cursus: IntraUserCursus!, primary: UIColor, isController: Bool) {
        ...
        if isController {
            ...
            self.userSearchField = UserSearchFieldView(user: .init(id: user.id, login: user.login), primary: primary)
        }
        else {
            self.antenneView = AntenneBlurredView(isBreak: false, isAntenneAnimating: true)
        ...
        if isController {
            ...
            self.userSearchField.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: App.safeAeraMain.left + HomeLayout.margin).isActive = true
            self.userSearchField.topAnchor.constraint(equalTo: self.topAnchor, constant: App.safeAeraMain.top + HomeLayout.margin).isActive = true
            self.userSearchField.trailingAnchor.constraint(equalTo: self.trailingAnchor, constant: -(App.safeAeraMain.right + HomeLayout.margin)).isActive = true
        }
        self.addSubview(self.cursusSelector)
        self.cursusSelector.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: App.safeAeraMain.left + HomeLayout.margin).isActive = true
        if isController {
            ...
            self.cursusSelector.topAnchor.constraint(equalTo: self.topAnchor, constant: HomeLayout.headerWithActionControllerViewHeigth + App.safeAera.top + HomeLayout.margin).isActive = true
        }
        self.cursusSelector.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: App.safeAeraMain.left + HomeLayout.margin).isActive = true
        self.addSubview(self.searchField)
        self.searchField.trailingAnchor.constraint(equalTo: self.trailingAnchor, constant: -(App.safeAeraMain.right + HomeLayout.margin)).isActive = true
    ...
    @MainActor private func startBuildingHolyGraph() async {
        ...
        if self.userSearchField == nil {
            offsetY = HomeLayout.headerWithActionViewHeigth + App.safeAera.top + HomeLayout.margin
        }
        else {
            offsetY = App.safeAeraMain.top + HomeLayout.roundedGenericActionsViewHeigth * 2.0 + HomeLayout.margin * 4.0
        }
        height = UIScreen.main.bounds.height - (offsetY + App.safeAera.bottom)
        do {
    ...
    @objc private func scrollViewContentTapped(sender: UITapGestureRecognizer) {
        ...
        if let view = self.scrollViewContainer.subviews.last(where: { $0.point(inside: $0.convert(location, from: sender.view!), with: nil) }) as? ProjectView {
            ...
            projectInfoView.frame = .init(x: HomeLayout.margin,
                                          ...
                                          height: UIScreen.main.bounds.height - (App.safeAera.top + App.safeAera.bottom + offset + HomeLayout.margin))
    ...
    func keyboardWillShow(curve: UIView.AnimationCurve, duration: TimeInterval, frame: CGRect) {
        guard self.searchProjectsView == nil else { return }
...
private extension HolyGraphView {
    ...
    final private class ProjectInfoView: HomePresentableVisualEffectView {
        ...
        @objc private func seeButtonTapped(sender: UITapGestureRecognizer) {
            ...
            (self.parentViewController as! HomeViewController).presentWithBlur(userProjectVC) {
        ...
        @objc private func seePeopleButtonTapped(sender: UITapGestureRecognizer) {
            ...
            (self.parentViewController as! HomeViewController).presentWithBlur(userList)
        }
