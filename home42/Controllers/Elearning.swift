...
final class ElearningViewController: HomeViewController, SearchFieldViewDelegate, UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate {
    ...
    required init() {
        ...
        self.subnotionTableView.contentInsetAdjustTopConstant(App.safeAeraMain.top + HomeLayout.smargin, bottom: App.safeAera.bottom + 40.0)
        ...
        self.searchField.topAnchor.constraint(equalTo: self.notionView.topAnchor, constant: App.safeAeraMain.top + HomeLayout.margin).isActive = true
        self.searchField.leadingAnchor.constraint(equalTo: self.notionView.leadingAnchor, constant: App.safeAeraMain.left + HomeLayout.margin).isActive = true
        self.searchField.trailingAnchor.constraint(equalTo: self.notionView.trailingAnchor, constant: -(App.safeAeraMain.left + HomeLayout.margin)).isActive = true
        ...
        self.notionTableView.contentInsetAdjustTopConstant(HomeLayout.roundedGenericActionsViewRadius + HomeLayout.smargin, bottom: App.safeAera.bottom + 40.0)
        ...
        self.subnotionTableView.leadingAnchor.constraint(equalTo: self.subnotionView.leadingAnchor, constant: App.safeAeraMain.left + HomeLayout.margin).isActive = true
        self.subnotionTableView.trailingAnchor.constraint(equalTo: self.subnotionView.trailingAnchor, constant: -(App.safeAeraMain.left + HomeLayout.margin)).isActive = true
    ...
    func notionSelected(_ notion: IntraNotion) {
        ...
        if notion.subnotions.count == 0 {
            DynamicAlert.init(contents: [.title(notion.name), .text(~"NOTION_HAVE_NO_SUBNOTIONS")], actions: [.normal(~"OK", nil)])
            return
        }
    ...
    private final class SubnotionPDFCell: SubnotionCell {
        ...
        override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
            self.pdfLinkLabel = BasicUILabel(text: ~"OPEN")
        ...
        @objc private func tapGesture() {
            guard let url = self.pdfAttachment.pdfUrl else {
                DynamicAlert(contents: [.title(~"ELEARNING_INCORRECT_LINK"), .text(self.pdfAttachment.url)], actions: [.normal(~"OK", nil)])
                return
            }
    ...
    private final class SubnotionLinkCell: SubnotionCell {
        ...
        override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
            self.openWebLink = BasicUILabel(text: ~"OPEN_WEB_LINK")
            ...
            self.openWebLinkSafari = BasicUILabel(text: ~"OPEN_WEB_LINK_SAFARI")
            ...
            self.copy = BasicUILabel(text: ~"COPY")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        func playVideo() {
            ...
            let source = (App.settings.elearningHD ? video.videoUrls.high_d : video.videoUrls.url).addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
            guard let url = URL(string: source) else {
                DynamicAlert(contents: [.title(~"ELEARNING_INCORRECT_LINK"), .text(source)], actions: [.normal(~"OK", nil)])
                return
            }
