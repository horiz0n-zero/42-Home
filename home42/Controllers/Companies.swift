...
final class CompaniesViewController: HomeViewController {
    ...
    required init() {
        ...
        self.tableView.contentInsetAdjustTopConstant(App.safeAeraMain.top + HomeLayout.smargin, bottom: App.safeAeraMain.bottom + HomeLayout.smargin)
    ...
    private func offerSelected(_ offer: IntraOffer) {
        ...
        DynamicAlert(.none, contents: [.titleWithPrimary(offer.title, HomeDesign.primary), .text(offer.big_description)], actions: [.normal(~"OK", nil), .highligth(~"SEE", seeDetails)])
    }
