...
final class EventsViewController: HomeViewController {
    ...
    required init() {
        ...
        Task.init(priority: .userInitiated, operation: { await self.tableView.nextPage() })
    }
    ...
    static func refreshUserEvents() async throws -> ContiguousArray<IntraUserEvent> {
        let today = Date()
        let events: ContiguousArray<IntraUserEvent> = try await HomeApi.get(.usersWithUserIdEventsUsers(App.user.id),
                                                                            params: ["sort":"-created_at", "page[size]": 30])
        
        //HomeDefaults[.userEvents] = events 
        return events.filter({ $0.event.beginDate >= today })
    }
    
    private func eventSelected(_ event: IntraEvent) {
        var actions: [DynamicAlert.Action] = [.normal(~"OK", nil)]
        ...
        if let userEvent = userSubscribed {
            ...
            actions.append(.highligth(~"EVENT_ACTION_UNREGISTER", {
            ...
            actions.append(.highligth(~"EVENT_ACTION_REGISTER", {
    ...
    final class EventInfiniteRequestTableView: GenericSingleInfiniteRequestTableView<EventTableViewCell, IntraEvent> {
        ...
        var userEvents: ContiguousArray<IntraUserEvent> = HomeDefaults.read(.userEvents) ?? []
    ...
    final class EventTableViewCell: BasicUITableViewCell, GenericSingleInfiniteRequestCell {
        ...
        func fill(with event: IntraEvent, userSubcribed: Bool) {
            ...
            if event.max_people != nil {
                if userSubcribed {
                    peopleText = ~"EVENT_STATE_REGISTERED" + " \(event.nbr_subscribers) / \(event.max_people!)"
                }
                else if event.max_people == event.nbr_subscribers {
                    peopleText = ~"EVENT_STATE_FULL" + " \(event.nbr_subscribers) / \(event.max_people!)"
                }
                else {
                ...
                if userSubcribed {
                    peopleText = ~"EVENT_STATE_REGISTERED" + " \(event.nbr_subscribers)"
                }
                else {
