...
final class ProfilViewController: HomeViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .events:
            ...
            if indexPath.row == 1 && (self.userEvents == nil || self.userEvents!.count == 0) {
                ...
                cell.update(with: self.userEvents == nil ? ~"LOADING" : ~"NO_FUTUR_EVENT", primary: self.currentPrimary)
                return cell
            }
        ...
        case .partnerships:
            if indexPath.row == 0 {
                return headerWithTitle(~"TITLE_PARTNERSHIPS")
            }
        ...
        case .titles:
            if indexPath.row == 0 {
                return headerWithTitle(~"TITLE_TITLES")
            }
        ...
        case .expertises:
            if indexPath.row == 0 {
                return headerWithTitle(~"TITLE_EXPERTISES")
            }
        ...
        case .achievements:
            if indexPath.row == 0 {
                return headerWithTitle(~"TITLE_ACHIEVEMENTS")
            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .events:
            ...
            DynamicAlert.init(.event(userEvent.event), contents: [.text(userEvent.event.eventDescription)], actions: [.normal(~"OK", nil), .highligth(~"EVENT_ACTION_UNREGISTER", {
        ...
        case .titles:
            ...
            let userList = UsersListViewController(.titlesWithTitleIdUsers(title.id), primary: self.currentPrimary, extra: .title(title))
            
            self.presentWithBlur(userList)
        case .expertises:
            guard indexPath.row != 0 else { return }
            let expertise = self.user.expertises_users[indexPath.row &- 1]
            let usersList = UsersListViewController(.expertisesWithExpertiseIdUsers(expertise.expertise_id), primary: self.currentPrimary, extra: .expertise(expertise.expertise_id))
            
            self.presentWithBlur(usersList)
        case .achievements:
            /*
            let achievement = self.user.achievements[indexPath.row]
            let usersList = UsersListViewController(.achievementsWithAchievementIdAchievementsUsers(achievement.id), primary: self.currentPrimary)
            
            usersList.headerTitle = achievement.name
            self.presentWithBlur(usersList)*/ /* Extremely breaked api users root */ break
        default:
            break
        }
    }
    ...
    required init() {
        ...
        self.tableView.contentInset = UIEdgeInsets(top: 0.0, left: 0.0, bottom: App.safeAera.bottom, right: 0.0)
    ...
    @MainActor func setupWithUser(_ login: String, id: Int) async {
        ...
        do {
            if id == 20091 {
                let emitterView = ParticlesEmitterView(.stars)
                
                emitterView.frame = self.headerCell.userIcon.frame
                self.headerCell.contentView.insertSubview(self.headerCell.userIcon, aboveSubview: emitterView)
            }
    ...
    @MainActor func setupWithMe() async {
        ...
        closeButton.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(actionCloseTapped(sender:))))
        self.headerCell.actionsStack.addArrangedSubview(closeButton)
        logoutButton.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(actionLogoutTapped(sender:))))
        self.headerCell.actionsStack.addArrangedSubview(logoutButton)
        settingsButton.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(actionSettingsTapped(sender:))))
    ...
    @objc private func cursusButtonTapped(sender: UITapGestureRecognizer) {
        let block: (Int, String) -> Void = { index, _ in
            ...
            if self.isMyProfil {
                App.userCursus = self.currentCursus
                App.userCoalition = self.currentCoalition
            }
        ...
        DynamicAlert(.withPrimary(~"INFO_CURSUS", self.currentPrimary),
                     ...
                     actions: [.normal(~"CANCEL", nil), .getRoulette(~"SELECT", block)])
    }
    
    @objc private func headerCoalitionIconTapped(sender: UITapGestureRecognizer) {
        guard self.headerCell.coalitionImage.image != HomeResources.svgLogo42, self.currentCursus != nil, self.currentCoalition != nil else {
            return
        }
    ...
    @objc private func actionLogoutTapped(sender: UITapGestureRecognizer) {
        ...
        DynamicAlert.init(.withPrimary(~"WARNING", self.currentPrimary),
                          contents: [.title(~"DISCONNECT_TITLE"), .text(~"DISCONNECT_MESSAGE")],
                          actions: [.normal(~"CANCEL", nil), .highligth(~"DISCONNECT", logout)])
    }
    ...
    @objc private func actionHolyGraphTapped(sender: UITapGestureRecognizer) {
        ...
        if self.currentCursus == nil {
            DynamicAlert(contents: [.text(String(format: ~"GRAPH_REQUIRED_CURSUS", self.user.login))], actions: [.normal(~"OK", nil)])
        }
        else {
        ...
        if App.settings.peopleExtraList1 != nil || App.settings.peopleExtraList2 != nil {
            ...
            if let people = peoples[self.user.login] {
                DynamicAlert.init(.withPrimary(people.list.title, self.currentPrimary),
                                  contents: [.title(String(format: ~"PEOPLE_REMOVE", people.login))],
                                  actions: [.normal(~"CANCEL", nil), .highligth(~"REMOVE", remove)])
            }
            else {
                func add(listType: People.ListType) {
                    peoples[self.user.login] = People(id: self.user.id, login: self.user.login, list: listType)
                    HomeDefaults.save(peoples, forKey: .peoples)
                    updateCluster()
                }
                
                actions = [.title(String(format: ~"PEOPLE_ADD", self.user.login)), .separator,
                           .normalWithPrimary(~"PEOPLE_FRIENDS", .actionPeople, People.ListType.friends.color, { add(listType: .friends) })]
            ...
            func add() {
                peoples[self.user.login] = People(id: self.user.id, login: self.user.login, list: .friends)
                HomeDefaults.save(peoples, forKey: .peoples)
                updateCluster()
            }
            
            if peoples[self.user.login] == nil {
                DynamicAlert(.fullPrimary(~"PEOPLE_FRIENDS", HomeDesign.greenSuccess),
                             contents: [.text(String(format: ~"PEOPLE_FRIENDS_ADD", self.user.login))],
                             actions: [.normal(~"CANCEL", nil), .highligth(~"ADD", add)])
            }
            else {
                DynamicAlert(.fullPrimary(~"PEOPLE_FRIENDS", HomeDesign.redError),
                             contents: [.text(String(format: ~"PEOPLE_FRIENDS_REMOVE", self.user.login))],
                             actions: [.normal(~"CANCEL", nil), .highligth(~"REMOVE", remove)])
            }
        }
    }
    ...
    @objc private func locationTapped(sender: UITapGestureRecognizer) {
        if self.currentCampus.campus_id != App.userCampus.campus_id {
            DynamicAlert(contents: [.text(~"CLUSTER_UNAVAILABLE_CAMPUS")], actions: [.normal(~"I_UNDERSTAND", nil)])
            return
        }
        if let cluster = App.mainController.controller as? ClustersViewController {
            if cluster.focusOnClusterView(with: self.user.location) == false {
                DynamicAlert(contents: [.text(String(format: ~"CLUSTER_NOFOCUS_FOR_HOST", self.user.login, self.user.location))], actions: [.normal(~"I_UNDERSTAND", nil)])
            }
            else {
    ...
    @objc private func correctionPointsHistoric(sender: UITapGestureRecognizer) {
        self.presentWithBlur(EvaluationPointsHistoricViewController(userId: self.user.id, userLogin: self.user.login, primary: self.currentPrimary), completion: nil)
    }
...
fileprivate extension ProfilViewController {
    ...
    final private class HeaderTableViewCell: BasicUITableViewCell {
        ...
        fileprivate let coalitionBackground: BasicUIImageView
        fileprivate let coalitionImage: SVGKFastImageView
        ...
        final private class InfosView: BasicUIView {
            ...
            final class InfoView: BasicUIVisualEffectView, ViewRecyclable {
                ...
                init(data: InfoView.Data) {
                    ...
                    self.actionButtonView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(InfoView.actionButtonTapped(sender:))))
                }
                required init?(coder: NSCoder) { fatalError("init(coder:) has not been implemented") }
            ...
            @frozen enum ContentType: String {
                case wallet = "INFO_WALLET"
                case evaluationPoints = "INFO_EVALUATION_POINTS"
                case location = "INFO_LOCATION"
                case phone = "INFO_PHONE"
                case cursus = "INFO_CURSUS"
                case pool = "INFO_POOL"
                case campus = "INFO_CAMPUS"
            }
            override init() {
        ...
        init() {
            ...
            self.coalitionImage = SVGKFastImageView(svgkImage: HomeResources.svgUnknowCoalition)
            self.coalitionImage.image.fillWith(color: HomeDesign.white)
        ...
        override func willMove(toSuperview newSuperview: UIView?) {
            ...
            self.actionsStack.topAnchor.constraint(equalTo: self.contentView.topAnchor, constant: App.safeAera.top + HomeLayout.margin).isActive = true
        ...
        func update(with user: IntraUser, coalition: IntraCoalition?, parent: ProfilViewController) {
            ...
            self.userIcon.update(with: user.login)
            if let coalition = coalition {
                ...
                if let image = HomeResources.storageSVGCoalition.get(coalition) {
                    image.fillWith(color: HomeDesign.white)
                    self.coalitionImage.image = image
                }
                else {
                    self.coalitionImage.image = HomeResources.svgUnknowCoalition
                    Task.init(priority: .userInitiated, operation: {
            ...
            if user.location != nil && App.userCampus.campus_id == parent.currentCampus.campus_id && App.mainController.controller is ClustersViewController {
...
extension ProfilViewController {
    ...
    final private class UserEventsTableViewCell: BasicUITableViewCell {
        ...
        override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
            self.title = LeftCurvedTitleView(text: ~"TITLE_EVENTS", primaryColor: HomeDesign.primary, addTopCorner: false)
...
extension ProfilViewController {
    ...
    final private class CorrectionsTableViewCell: BasicUITableViewCell {
        ...
        override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
            self.title = LeftCurvedTitleView(text: ~"TITLE_CORRECTIONS", primaryColor: HomeDesign.primary, addTopCorner: false)
            self.logsButton = .init(asset: .actionHistoric, color: HomeDesign.primary)
        ...
        override func willMove(toSuperview newSuperview: UIView?) {
            ...
            self.view.topAnchor.constraint(equalTo: self.title.bottomAnchor, constant: HomeLayout.margins).isActive = true
    ...
    final class CorrectionsView: BasicUIView, UITableViewDelegate, UITableViewDataSource {
        ...
        final private class Cell: BasicUIView, GenericTableViewCellView {
            ...
            private let correctorIcon: UserProfilIconView = UserProfilIconView()
            ...
            private let dateLabel: BasicUILabel = BasicUILabel(text: "???")
            ...
            override init() {
                ...
                self.dateLabel.font = HomeLayout.fontBoldMedium
            ...
            override func willMove(toSuperview newSuperview: UIView?) {
                ...
                self.correctedContainer.heightAnchor.constraint(equalToConstant: HomeLayout.userProfilIconHeigth).isActive = true
                self.correctedContainer.widthAnchor.constraint(greaterThanOrEqualToConstant: HomeLayout.userProfilIconHeigth).isActive = true
                // self.correctedContainer.trailingAnchor.constraint(equalTo: self.container.trailingAnchor, constant: -HomeLayout.smargin).isActive = true
                self.container.addSubview(self.dateLabel)
                self.dateLabel.leadingAnchor.constraint(equalTo: self.correctedContainer.trailingAnchor).isActive = true
                self.dateLabel.centerYAnchor.constraint(equalTo: self.correctorIcon.centerYAnchor).isActive = true
                self.dateLabel.trailingAnchor.constraint(equalTo: self.container.trailingAnchor, constant: -HomeLayout.margin).isActive = true
            ...
            func update(with scaleTeam: IntraScaleTeam, primary: UIColor) {
                ...
                var recycledCorrecteds = self.correctedContainer.subviews as! [UserProfilIconView]
                var correctedIcon: UserProfilIconView
                ...
                if let login = scaleTeam.corrector?.login {
                    self.correctorIcon.update(with: login)
                    correctorLogin = login
                }
                else {
                ...
                for corrected in scaleTeam.correcteds {
                    if recycledCorrecteds.count > 0 {
                        ...
                        correctedIcon.update(with: corrected.login)
                    }
                    else {
                        correctedIcon = UserProfilIconView(login: corrected.login)
                    ...
                    correctedIcon.setSize(HomeLayout.userProfilIconHeigth, HomeLayout.userProfilIconRadius)
                ...
                if scaleTeam.correcteds.count == 0 {
                    if recycledCorrecteds.count > 0 {
                        ...
                        correctedIcon = UserProfilIconView()
                        correctedIcon.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(CorrectionsView.Cell.iconTapped(sender:))))
                    }
                    self.correctedContainer.addSubview(correctedIcon)
                    correctedIcon.leadingAnchor.constraint(equalTo: leadingAnchor, constant: HomeLayout.dmargin).isActive = true
                ...
                self.dateLabel.text = self.scaleTeam.beginAtDate.newDiffTime(to: Date())
                if scaleTeam.corrector.id == App.user.id {
                    self.detailsLabel.text = String(format: ~"YOU_WILL_CORRECT", scaleTeam.team.name, projectName)
                }
                else {
                    self.detailsLabel.text = String(format: ~"YOU_WILL_BE_CORRECTED", scaleTeam.correcteds.first?.login ?? "???", projectName)
                }
                self.primary = primary
                self.dateLabel.textColor = self.primary
                    ...
                    Task.init(priority: .userInitiated, operation: {
                        do {
                            ...
                            if let userProject = user.projects_users.first(where: { $0.project.id == self.scaleTeam.team.project_id }) {
                                ...
                                (self.parentViewController as! HomeViewController).presentWithBlur(vc)
                            }
                        }
                        catch {
                            DynamicAlert.presentWith(error: error as! HomeApi.RequestError)
            ...
            @objc private func iconTapped(sender: UITapGestureRecognizer) {
                ...
                if self.scaleTeam.corrector.login == login {
                    ...
                    id = self.scaleTeam.correcteds.first(where: { $0.login == login })?.id
                }
                if id != nil {
                    profil = ProfilViewController()
                    Task.init(priority: .userInitiated, operation: {
                        await profil.setupWithUser(login!, id: id)
                    })
                    (self.parentViewController as! HomeViewController).presentWithBlur(profil, completion: nil)
                }
            }
        }
        ...
        func refreshUserCorrections() throws {
            Task.init(priority: .userInitiated, operation: {
                ...
                let requestCount = self.isSizeLimited ? 15 : App.settings.profilCorrectionsCount
                
                self.scaleTeams = try await HomeApi.get(.usersUserIdScaleTeams(App.user.id),
                                                        params: ["page[size]": requestCount,
                                                                 "sort": "-begin_at"])//,
                                                                 //"filter[filled]": false])
                ...
                if self.scaleTeams.count == 0 {
                    ...
                    if self.isSizeLimited && self.scaleTeams.count > 1 {
                        ...
                        self.tableView.isScrollEnabled = false
                        newHeight = HomeLayout.userProfilCorrectionViewCellHeight * CGFloat(self.scaleTeams.count)
                    }
                }
                ...
                if let cell: UITableViewCell = self.parent(), let table: UITableView = cell.parent() {
                    ...
                    HomeAnimations.animateMedium({
                        self.tableViewHeight.constant = newHeight
                        self.parentViewController.view.layoutIfNeeded() // crash make optional parentviewcontroller...
                    })
                }
            })
        }
        ...
        func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
            ...
            return self.scaleTeams.count
        }
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            if self.scaleTeams.count == 0 {
                let cell = tableView.dequeueReusableCell(withIdentifier: "message", for: indexPath) as! MessageTableViewCell
                
                cell.update(with: self.isLoading ? ~"LOADING" : ~"NO_FUTUR_CORRECTION", primary: self.primary)
                return cell
            }
            else {
...
private extension ProfilViewController {
    ...
    final private class ProjectsTableViewCell: BasicUITableViewCell, UITableViewDelegate, UITableViewDataSource, SegmentViewDelegate {
        ...
        override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
            self.title = LeftCurvedTitleView(text: ~"PROJECTS", primaryColor: HomeDesign.primary, addTopCorner: false)
            self.segment = SegmentView(values: [~"PROJECTS_INRUN", ~"PROJECTS_ENDED"], selectedIndex: 0)
            self.tableView = BasicUITableView()
        ...
        func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            ...
            (self.parentViewController as! HomeViewController).presentWithBlur(viewController)
        }
    }
}
...
fileprivate extension ProfilViewController {
    ...
    final private class SkillsTableViewCell: BasicUITableViewCell {
        ...
        override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
            self.title = LeftCurvedTitleView(text: ~"SKILLS", primaryColor: HomeDesign.primary, addTopCorner: false)
            super.init(style: style, reuseIdentifier: reuseIdentifier)
        ...
        func update(with skills: [IntraUserSkill], primary: UIColor) {
            ...
            self.title.update(with: ~"SKILLS", primaryColor: primary, animate: false)
        ...
        override func draw(_ rect: CGRect) {
            ...
            self.primaryColor.withAlphaComponent(0.75).setFill()
...
private extension ProfilViewController {
    ...
    final private class ExpertiseView: BasicUIView, HomeFramingTableViewCellView {
        ...
        private let name: BasicUILabel
        private let stars: BasicUILabel
        ...
        func update(with expertise: IntraUserExpertise, primary: UIColor) {
            ...
            self.stars.text = String(repeating: "⭐️", count: expertise.value)
            self.backgroundColor = primary.withAlphaComponent(HomeDesign.alphaLowLayer)
        }
    }
    ...
    final private class AchievementView: BasicUIView, HomeFramingTableViewCellView {
        ...
        private let svgView: SVGKFastImageView
        private let nameLabel: BasicUILabel
        private let descriptionContainer: BasicUIView
        private let descriptionLabel: BasicUILabel
        
        override init() {
            self.svgView = SVGKFastImageView(svgkImage: nil)
            self.svgView.contentMode = .scaleAspectFill
            self.svgView.layer.cornerRadius = HomeLayout.scorner
            self.svgView.layer.masksToBounds = true
            self.svgView.translatesAutoresizingMaskIntoConstraints = false
        ...
        override func willMove(toSuperview newSuperview: UIView?) {
            ...
            self.svgView.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: HomeLayout.margin).isActive = true
            self.svgView.topAnchor.constraint(equalTo: self.topAnchor, constant: HomeLayout.margin).isActive = true
            self.svgView.heightAnchor.constraint(equalToConstant: HomeLayout.profilAchievementImageSize).isActive = true
            self.svgView.widthAnchor.constraint(equalToConstant: HomeLayout.profilAchievementImageSize).isActive = true
            self.addSubview(self.nameLabel)
            self.nameLabel.leadingAnchor.constraint(equalTo: self.svgView.trailingAnchor, constant: HomeLayout.margin).isActive = true
            self.nameLabel.centerYAnchor.constraint(equalTo: self.svgView.centerYAnchor).isActive = true
            self.nameLabel.topAnchor.constraint(lessThanOrEqualTo: self.svgView.topAnchor).isActive = true
            self.nameLabel.bottomAnchor.constraint(lessThanOrEqualTo: self.svgView.bottomAnchor).isActive = true
            ...
            self.descriptionContainer.topAnchor.constraint(equalTo: self.svgView.bottomAnchor, constant: HomeLayout.margin).isActive = true
        ...
        private func imageReceived(_ id: String, image: SVGKImage) {
            if id == "\(self.achievement.id)" {
                self.svgView.image = image
            }
        }
        ...
        func update(with achievement: IntraUserAchievement, primary: UIColor) {
            ...
            self.backgroundColor = (achievement.visible ? primary : HomeDesign.gold).withAlphaComponent(HomeDesign.alphaLowLayer)
            self.descriptionContainer.backgroundColor = self.backgroundColor
        }
    }
}
