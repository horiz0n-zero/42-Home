...
final class UserEventsHistoricViewController: HomeViewController, UserSearchFieldViewDelegate {
    ...
    init(user: IntraUser, primary: UIColor) {
        self.header = HeaderWithActionsView(title: ~"TITLE_EVENTS", actions: nil)
        self.userSearchFieldView = UserSearchFieldView(user: .init(id: user.id, login: user.login), primary: primary)
    ...
    private func eventSelected(_ event: IntraEvent) {
        DynamicAlert.init(.event(event), contents: [.text(event.eventDescription)], actions: [.normal(~"OK", nil)])
    }
