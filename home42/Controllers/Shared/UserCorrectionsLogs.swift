//
//  UserCorrectionsLogs.swift
//  home42
//
//  Created by Antoine Feuerstein on 01/05/2021.
//
...
final class UserCorrectionsLogsViewController: HomeViewController, SegmentViewDelegate, UserSearchFieldViewDelegate {
    ...
    @frozen enum Selection: Int {
        ...
        static var keys: [String] {
            return [~"AS_CORRECTOR", ~"AS_CORRECTED"]
        }
    }
    ...
    init(user: IntraUser, primary: UIColor = HomeDesign.primary) {
        self.header = HeaderWithActionsView(title: ~"TITLE_CORRECTIONS")
    ...
    final class ScaleTeamView: BasicUIView, HomeFramingTableViewCellView {
        ...
        private let starsLabel: BasicUILabel
        ...
        override init() {
            ...
            self.starsLabel = BasicUILabel(text: "???")
            self.starsLabel.textAlignment = .left
            self.starsLabel.font = HomeLayout.fontSemiBoldNormal
        ...
        override func willMove(toSuperview newSuperview: UIView?) {
            ...
            self.correctorComment.topAnchor.constraint(equalTo: self.correctorIcon.bottomAnchor, constant: HomeLayout.margins).isActive = true
            ...
            self.correctedIcons.topAnchor.constraint(equalTo: self.correctedFeedback.bottomAnchor, constant: HomeLayout.margin).isActive = true
            ...
            self.starsLabel.centerYAnchor.constraint(equalTo: self.correctedFeedback.bottomAnchor).isActive = true
            self.starsLabel.leadingAnchor.constraint(equalTo: self.correctedFeedback.leadingAnchor).isActive = true
        }
        ...
        func update(with scaleTeam: IntraScaleTeam) {
            ...
            self.correctorIcon.update(with: scaleTeam.corrector.login)
            ...
            self.starsLabel.text = String(repeating: "⭐️", count: scaleTeam.feedbacks.first?.rating ?? 1)
            ...
            for (index, corrected) in scaleTeam.correcteds.enumerated() {
                if index >= self.correctedIcons.arrangedSubviews.count {
                    userProfilIconView = UserProfilIconView(login: corrected.login)
                    ...
                    userProfilIconView.update(with: corrected.login)
                }
            }
