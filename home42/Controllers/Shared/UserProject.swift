...
final class UserProjectViewController: HomeViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    init(user: IntraUser, userProject: IntraUserProject, primary: UIColor = HomeDesign.primary) {
        ...
        self.tableView.contentInset = .init(top: 0.0, left: 0.0, bottom: App.safeAeraMain.bottom + HomeLayout.margin, right: 0.0)
    ...
    @MainActor private func startGettingProject() async throws {
        let params = self.userProject.current_team_id == nil ? ["filter[user_id]": self.user.id] : ["filter[team_id]":self.userProject.current_team_id!]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .scaleTeams:
            if indexPath.row == 0 {
                ...
                cell.update(with: .init(id: self.user.id, login: self.user.login), project: self.userProject, scaleTeams: self.scaleTeams)
                return cell
            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let userProject = self.user.projects_users.first(where: { $0.project.id == link.id }) {
            ...
            DynamicAlert(contents: [.text(String(format: ~"USER_PROJECTS_CHILDREN_NOT_FOUND", link.name, self.user.login))], actions: [.normal(~"OK", nil)])
        }
    }
    
    final private class ScaleTeamHeaderTableViewCell: BasicUITableViewCell {
        
        private let groupNameLabel: BasicUILabel
        private let statusView: StatusWithTeamMemberView
        
        override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
            self.groupNameLabel = BasicUILabel(text: "")
            self.groupNameLabel.font = HomeLayout.fontSemiBoldTitle
            self.groupNameLabel.textColor = HomeDesign.black
            self.groupNameLabel.textAlignment = .center
        ...
        override func willMove(toSuperview newSuperview: UIView?) {
            ...
            self.groupNameLabel.topAnchor.constraint(equalTo: self.contentView.topAnchor).isActive = true
            self.groupNameLabel.leadingAnchor.constraint(equalTo: self.contentView.leadingAnchor, constant: HomeLayout.margin).isActive = true
            self.groupNameLabel.trailingAnchor.constraint(equalTo: self.contentView.trailingAnchor, constant: -HomeLayout.margin).isActive = true
            self.contentView.addSubview(self.statusView)
            self.statusView.topAnchor.constraint(equalTo: self.groupNameLabel.bottomAnchor, constant: HomeLayout.margin).isActive = true
        ...
        func update(with user: IntraUserInfo, project: IntraUserProject, scaleTeams: ContiguousArray<IntraScaleTeam>) {
            if let first = scaleTeams.first {
                self.groupNameLabel.text = first.team.name
            }
        ...
        final private class StatusWithTeamMemberView: BasicUIView, UITableViewDelegate, UITableViewDataSource {
            ...
            final class SmartUserInfoView: BasicUITableViewCell {
                ...
                func update(with userInfo: IntraUserInfo) {
                    self.profilIcon.update(with: userInfo.login)
