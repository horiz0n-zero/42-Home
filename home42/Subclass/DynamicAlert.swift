...
final class DynamicAlert: DynamicController, Keyboard {
    ...
    @frozen enum Action {
        ...
        var isHighligth: Bool {
            switch self {
            case .highligth(_, _), .getCode(_, _), .getRoulette(_, _), .apiErrorJSON, .slotInterval(_), .getIcon(_, _), .textEditor(_):
                return true
            default:
                return false
            }
        ...
        var text: String {
            ...
            case .normal(let txt, _), .highligth(let txt, _), .getCode(let txt, _), .getRoulette(let txt, _), .getIcon(let txt, _):
                return txt
            case .slotInterval(_), .textEditor(_):
                return ~"SELECT"
            case .apiErrorJSON:
                return "JSON"
            }
    ...
    @discardableResult init(_ style: DynamicAlert.Style = .primary(~"WARNING"), contents: [Content], actions: [Action]) {
        super.init()
        ...
        contentView.topAnchor.constraint(greaterThanOrEqualTo: self.view.safeAreaLayoutGuide.topAnchor, constant: HomeLayout.margins).isActive = true
        //contentView.bottomAnchor.constraint(greaterThanOrEqualTo: self.view.safeAreaLayoutGuide.bottomAnchor, constant: -HomeLayout.margins).isActive = true
        ...
        for content in contents {
            ...
            case .usersSelector(let user, let block):
                ...
                usersSelector.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: HomeLayout.margin).isActive = true
    ...
    func keyboardWillShow(curve: UIView.AnimationCurve, duration: TimeInterval, frame: CGRect) {
        //let bottom = self.view.constraints.first(where: { $0.relation == .greaterThanOrEqual && $0.firstAttribute == .bottom })!
        
        //centerY.priority = .defaultLow
        //self.keyboardElementBecomeVisible(on: bottom, view: self.view,
        //                                  keyboardFrame: frame, curve: curve, duration: duration)
        print(#function)
    }
    ...
    @MainActor @inlinable static func presentWith(error: HomeApi.RequestError) {
        ...
        if let data = error.data {
            actions = [.normal(~"OK", nil), .apiErrorJSON(data, error)]
        }
        else {
            actions = [.normal(~"OK", nil)]
        }
        DynamicAlert.init(contents: [.apiError(error)], actions: actions)
    }
    ...
    @inlinable static func presentInformation(_ asset: UIImage.Assets, title: String, description: String) {
        DynamicAlert(.none,
                     ...
                     actions: [.highligth(~"I_UNDERSTAND", nil)])
    }
}
// MARK: - event header
extension DynamicAlert {
    ...
    final private class EventHeader: BasicUIView {
        ...
        init(event: IntraEvent) {
            ...
            self.startDateLabel = BasicUILabel(text: ~"EVENT_HEADER_START_AT" + event.beginDate.toString(.custom("EEEE dd/MM/yyyy HH:mm")))
            ...
            self.endDateLabel = BasicUILabel(text: ~"EVENT_HEADER_END_AT" + event.endDate.toString(.custom("EEEE dd/MM/yyyy HH:mm")))
        ...
        override func willMove(toSuperview newSuperview: UIView?) {
            ...
            if let button = self.peopleButton {
                self.addSubview(button)
                button.trailingAnchor.constraint(equalTo: self.trailingAnchor, constant: -HomeLayout.margin).isActive = true
                button.centerYAnchor.constraint(equalTo: self.bottomAnchor).isActive = true
                button.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(usersTapped(gesture:))))
            }
        }
...
extension DynamicAlert {
    ...
    final private class IconsView: BasicUIView {
        ...
        override func willMove(toSuperview newSuperview: UIView?) {
            ...
            let availableWidth: CGFloat = (UIScreen.main.bounds.width - (App.safeAera.left + App.safeAera.right + HomeLayout.smargin + HomeLayout.margins))
...
extension DynamicAlert {
    ...
    final class TextView: BasicUITextView, UITextViewDelegate {
...
extension DynamicAlert {
    ...
    final private class SlotIntervalSelector: BasicUIView, SelectorViewDelegate {
        ...
        private var startError: Bool = false
        private let endDaySelector: SelectorView<Int>
        private let endHourSelector: SelectorView<Int>
        private let endMinuteSelector: SelectorView<Int> // del minutes
        private var endError: Bool = false
        ...
        override func willMove(toSuperview newSuperview: UIView?) {
            ...
            self.startHourSelector.leadingAnchor.constraint(equalTo: self.startDaySelector.trailingAnchor, constant: HomeLayout.smargin).isActive = true
            self.startHourSelector.centerYAnchor.constraint(equalTo: self.startDaySelector.centerYAnchor).isActive = true
            self.startHourSelector.widthAnchor.constraint(equalToConstant: 80.0).isActive = true
            self.addSubview(self.startMinuteSelector)
            self.startMinuteSelector.leadingAnchor.constraint(equalTo: self.startHourSelector.trailingAnchor, constant: HomeLayout.smargin).isActive = true
            self.startMinuteSelector.centerYAnchor.constraint(equalTo: self.startHourSelector.centerYAnchor).isActive = true
            self.startMinuteSelector.trailingAnchor.constraint(equalTo: self.trailingAnchor).isActive = true
            self.startMinuteSelector.widthAnchor.constraint(equalToConstant: 80.0).isActive = true
            self.addSubview(self.endDaySelector)
            self.endDaySelector.leadingAnchor.constraint(equalTo: self.leadingAnchor).isActive = true
            self.endDaySelector.topAnchor.constraint(equalTo: self.startDaySelector.bottomAnchor, constant: HomeLayout.margin).isActive = true
            self.addSubview(self.endHourSelector)
            self.endHourSelector.leadingAnchor.constraint(equalTo: self.endDaySelector.trailingAnchor, constant: HomeLayout.smargin).isActive = true
            self.endHourSelector.centerYAnchor.constraint(equalTo: self.endDaySelector.centerYAnchor).isActive = true
            self.endHourSelector.widthAnchor.constraint(equalToConstant: 80.0).isActive = true
            self.addSubview(self.endMinuteSelector)
            self.endMinuteSelector.leadingAnchor.constraint(equalTo: self.endHourSelector.trailingAnchor, constant: HomeLayout.smargin).isActive = true
            self.endMinuteSelector.centerYAnchor.constraint(equalTo: self.endHourSelector.centerYAnchor).isActive = true
            self.endMinuteSelector.trailingAnchor.constraint(equalTo: self.trailingAnchor).isActive = true
            self.endMinuteSelector.widthAnchor.constraint(equalToConstant: 80.0).isActive = true
            
            self.endMinuteSelector.bottomAnchor.constraint(equalTo: self.bottomAnchor).isActive = true
        }
        var startDate: Date {
        ...
        func selectorSelect<Int>(_ selector: SelectorView<Int>) {
            switch selector {
            case self.startDaySelector, self.startHourSelector, self.startMinuteSelector:
                
                
                self.changeStartStyle()
            default:
                self.endError = !self.endError
                self.changeEndStyle()
            }
        }
        
        func changeStartStyle() {
            let primary = self.startError ? HomeDesign.redError : HomeDesign.primary
        ...
        func changeEndStyle() {
            let primary = self.endError ? HomeDesign.redError : HomeDesign.primary
...
extension DynamicAlert {
    ...
    final private class ColorPicker: BasicUIView, ValueSelectorWithArrowsDelegate {
        ...
        init(color: UIColor, pointer: UnsafeMutablePointer<UIColor>) {
            ...
            self.redSelector = .init(min: 0, max: 255, step: 1, value: self.colorDescriptor.redInteger, textDisplay: .usingSuffix(~"COLOR_PICKER_RED" + ": "))
            self.greenSelector = .init(min: 0, max: 255, step: 1, value: self.colorDescriptor.greenInteger, textDisplay: .usingSuffix(~"COLOR_PICKER_GREEN" + ": "))
            self.blueSelector = .init(min: 0, max: 255, step: 1, value: self.colorDescriptor.blueInteger, textDisplay: .usingSuffix(~"COLOR_PICKER_BLUE" + ": "))
            super.init()
...
extension DynamicAlert {
    ...
    final private class UsersSelector: BasicUIView, SearchFieldViewDelegate {
        ...
        init(user: IntraUserInfo?, primary: UIColor, block: @escaping (IntraUserInfo) -> ()) {
            self.searchField = SearchFieldViewWithTimer(placeholder: ~"LOGIN")
            self.searchField.setPrimary(primary)
            if let user = user {
                self.tableView = .init(.users, parameters: ["search[login]": user.login, "sort":"-login"], page: 1, pageSize: 50, primary: primary)
                self.searchField.text = user.login
            }
            else {
                self.tableView = .init(.users, parameters: ["sort":"-login"], page: 1, pageSize: 50, primary: primary)
            }
            self.block = block
            super.init()
        ...
        override func willMove(toSuperview newSuperview: UIView?) {
            guard newSuperview != nil else { return }
            let h = self.tableView.heightAnchor.constraint(equalToConstant: UIScreen.main.bounds.height)
...
extension DynamicAlert {
    ...
    final private class TextEditor: RoundedGenericActionsView<BasicUITextField, ActionButtonView>, UITextFieldDelegate {
        ...
        init(defaultText: String, primary: UIColor) {
            ...
            textField.attributedPlaceholder = .init(string: ~"REPLACE", attributes: [.foregroundColor: HomeDesign.gray, .font: HomeLayout.fontRegularMedium])
