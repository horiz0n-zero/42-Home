//
//  RoundedGenericActionsView.swift
//  home42
//
...
final class SelectorView<E>: RoundedGenericActionsView<BasicUILabel, ActionButtonView> {
    ...
    @objc private func selectButtonTapped(sender: UITapGestureRecognizer) {
        ...
        if self.keys.count == 0 {
            DynamicAlert.init(.withPrimary(~"IMPOSSIBLE", primary), contents: [.text(~"NO_SELECTION_AVAILABLE")], actions: [.normal(~"OK", nil)])
        }
        else {
            DynamicAlert.init(.noneWithPrimary(primary), contents: [.roulette(self.keys, self.selectedIndex)], actions: [.getRoulette(~"SELECT", { index, text in
...
final class UserSearchFieldView: RoundedGenericActionsView<UserSearchFieldView.UserIconLoginView, ActionButtonView> {
    ...
    @objc private func tapGesture(sender: UITapGestureRecognizer) {
        ...
        DynamicAlert(.noneWithPrimary(self.primary), contents: [.usersSelector(self.user, block)], actions: [.normal(~"CANCEL", nil)])
    }
    ...
    final class UserIconLoginView: BasicUIView {
        ...
        init(user: IntraUserInfo?, primary: UIColor) {
            if let user = user {
                self.icon = UserProfilIconView(login: user.login)
                self.loginLabel = BasicUILabel(text: user.login)
            }
            else {
                self.icon = UserProfilIconView()
                self.loginLabel = BasicUILabel(text: ~"LOGIN") // make text grey placeholder ?
            }
        ...
        fileprivate func update(with user: IntraUserInfo?) {
            if let user = user {
                ...
                self.icon.update(with: user.login)
            }
            else {
