...
class ClusterViewClassicBase: UIImageView, ClusterView {
    ...
    func configure(location: IntraClusterLocation?, color: CGColor?) {
        ...
        if let user = location?.user {
            ...
            if let image = HomeResources.storageLoginImages.get(user.login) {
                self.image = image
            }
            else {
                self.image = UIImage.Assets.defaultLogin.image
                Task.init(priority: .userInitiated, operation: {
                    if let (login, image) = await HomeResources.storageLoginImages.obtain(user.login), login == self.location?.user.login {
                        self.image = image
                    }
                })
            }
