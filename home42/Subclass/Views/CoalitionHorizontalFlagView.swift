//
//  CoalitionHorizontalFlagView.swift
//  home42
//
//  Created by Antoine Feuerstein on 22/04/2021.
//
...
final class CoalitionHorizontalFlagView: BasicUIView {
    ...
    private let icon: SVGKFastImageView
    private let label: BasicUILabel
    
    override init() {
        self.icon = SVGKFastImageView(svgkImage: HomeResources.svgUnknowCoalition)
    ...
    func update(with coalition: IntraCoalition, position: Int, blocSize: Int) {
        ...
        if let image = HomeResources.storageSVGCoalition.get(coalition) {
            ...
            self.icon.image = HomeResources.svgUnknowCoalition
            self.icon.image.fillWith(color: HomeDesign.white)
