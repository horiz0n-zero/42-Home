...
final class DynamicActionsSheet: DynamicController {
    ...
    @discardableResult init(actions: [DynamicActionsSheet.Action], primary: UIColor = HomeDesign.primary) {
        ...
        self.cancelButton = DynamicActionsSheet.ButtonView(primary: primary, title: ~"CANCEL", asset: nil, block: nil)
        ...
        for action in actions {
            ...
            case .telephony(let phoneNumber):
                addActionView(DynamicActionsSheet.ButtonView(primary: primary, title: ~"CALL", asset: .actionSettings, block: { HomeTelephony.call(phoneNumber) }))
                addSeparator()
                addActionView(DynamicActionsSheet.ButtonView(primary: primary, title: ~"MESSAGE", asset: .actionSearch, block: { HomeTelephony.message(phoneNumber) }))
                addSeparator()
                addActionView(DynamicActionsSheet.ButtonView(primary: primary, title: ~"COPY", asset: .actionAdd, block: { UIPasteboard.general.string = phoneNumber }))
            case .separator:
                addSeparator()
            }
        }
    ...
    override func present() {
        ...
        self.cancelButton.center = .init(x: 0.0, y: App.safeAeraMain.bottom + HomeLayout.dynamicActionsSheetButtonHeigth)
        super.present()
